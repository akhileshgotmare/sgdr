...
parser.add_argument('--w1', type=str, default='results/1541640166128609/epoch_30.t7', help='name/path of file from where we load w1')
parser.add_argument('--w2', type=str, default='results/1541640166128609/epoch_70.t7', help='name/path of file from where we load w2')
...
def evaluate_curve():
    ...
    t_list = np.linspace(0, 1, 30)[1:29].tolist()
    ...
    for t_id, t in enumerate(t_list):
        ...
        '''
        (Batch Norm) section A.2 in MC paper | 
        Need to perform a forward pass over training set to record
        means and variances
        '''
        net5.train()
...
def save_log_n_model():
    ...
    np.save('./results/curve_find/' + unique_run_str+'/'+ 'curve_find_info.npy' ,infocurvefind)
    torch.save(net4,'./results/curve_find/'+unique_run_str+'/'+'_theta_step_'+str(args.numthetasteps)
               +'.t7')
...
for theta_step in range(args.numthetasteps): #500 steps correspond to 1 epoch for bsz = 100
    ...
    '''for resnet experiments (iclr rebuttal) we need note evaluate curve at intermediate steps'''
    if theta_step == 0: 
        evaluate_curve()
        save_log_n_model()
